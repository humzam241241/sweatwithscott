...
export function hasProgramAccess(userId: number, programSlug: string): boolean {
  const enrollments = (dbOperations as any).getUserProgramEnrollments?.(userId) as Array<{ slug?: string; status?: string }> | undefined;
  if (!enrollments?.length) return false;
  return enrollments.some((e) => e.slug === programSlug && (e.status ?? "active") === "active");
}
