...
function generateClassInstances() {
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const dayClasses = classes.filter((cls: any) => cls.day_of_week === dayName)
      ...
      dayClasses.forEach((cls: any) => {
        try {
          ...
          console.error(`Error creating instance for ${cls.name} on ${dateString}:`, error)
        }
      })
    }
...
function addSampleBookingsAndPayments() {
  try {
    ...
    const instances = db.prepare("SELECT * FROM class_instances LIMIT 10").all()
    ...
    users.forEach((user: any, userIndex: number) => {
      instances.slice(0, 3).forEach((instance: any, instanceIndex: number) => {
        ...
        try {
          const result = insertBooking.run(user.id, instance.id, "confirmed", paymentStatus, 25.0, attended)
          ...
          db.prepare("UPDATE class_instances SET current_bookings = current_bookings + 1 WHERE id = ?").run(instance.id)
        } catch (error) {
...
export const dbOperations = {
  ...
  getAllClasses: () => {
    try {
      return db.prepare("SELECT * FROM classes WHERE is_active = 1 ORDER BY name").all()
    } catch (error) {
  ...
  bookClass: (userId: number, classInstanceId: number) => {
    try {
      ...
      // Update class instance booking count
      db.prepare("UPDATE class_instances SET current_bookings = current_bookings + 1 WHERE id = ?").run(classInstanceId)
  ...
  cancelBooking: (userId: number, classInstanceId: number) => {
    try {
      ...
      if (result.changes > 0) {
        // Update class instance booking count
        db.prepare("UPDATE class_instances SET current_bookings = current_bookings - 1 WHERE id = ?").run(
          classInstanceId,
        )
      }
  ...
  markPaymentPaid: (bookingId: number, paymentMethod = "cash") => {
    try {
      const booking = db.prepare("SELECT * FROM class_bookings WHERE id = ?").get(bookingId)
      if (!booking) throw new Error("Booking not found")

      // Update booking payment status
      db.prepare("UPDATE class_bookings SET payment_status = 'paid' WHERE id = ?").run(bookingId)
