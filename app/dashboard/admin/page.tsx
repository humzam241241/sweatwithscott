...
function MembersCard() {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await fetch('/api/admin/members'); const j = await r.json(); setMembers(Array.isArray(j) ? j : []); } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);
      ...
      <div className="overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800">
            {rows.map(m => (
              <tr key={m.id} className={classNames(m.overdue && 'bg-red-900/20')}>
                ...
                <td className="p-2">
                  <span className={classNames('px-2 py-0.5 rounded border text-xs', m.paid ? 'text-green-400 border-green-400' : 'text-yellow-400 border-yellow-400')}>
                    {m.paid ? 'Paid' : 'Unpaid'}
                  </span>
                </td>
                <td className={classNames('p-2', m.overdue ? 'text-red-400' : 'text-gray-300')}>{m.subscription_status}</td>
                <td className="p-2">
                  <div className="flex gap-2">
