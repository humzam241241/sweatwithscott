        ...
        if (session.mode === "subscription" && session.subscription) {
          ...
          try {
            ...
            if (u?.email && (u.email_opt_in ?? 1)) {
              await sendMail({ to: u.email, subject: "Sweat with Scott – Subscription active", text: `Your subscription is now active. Welcome to Sweat with Scott Daily.` });
            }
          } catch {}
        }
        // Enroll online programs from checkout metadata
        const planCode = session.metadata?.planCode;
        if (planCode === "EIGHT_WEEK_RESET" || planCode === "DAILY_DRILLS_MONTHLY") {
          const program = (dbOperations as any).getProgramByPriceCode?.(planCode);
          if (program?.id) {
            (dbOperations as any).enrollUserInProgram?.(userId, Number(program.id), "active");
          }
        }
        ...
        if (session.mode === "payment" && session.metadata?.flow === "DROP_IN") {
          ...
          if (Number.isFinite(instanceId)) {
            ...
            try {
              const u = dbOperations.getUserById(userId) as any;
              if (u?.email && (u.email_opt_in ?? 1)) {
                await sendMail({ to: u.email, subject: "Sweat with Scott – Drop-in confirmed", text: `Thanks for your drop-in payment. See you in class!` });
              }
            } catch {}
          }
        }
        if (session.mode === "payment" && session.metadata?.flow === "EIGHT_WEEK_RESET") {
          try {
            const u = dbOperations.getUserById(userId) as any;
            if (u?.email && (u.email_opt_in ?? 1)) {
              await sendMail({ to: u.email, subject: "Sweat with Scott – 8-Week Reset unlocked", text: `Your 8-week program is now available in your member dashboard.` });
            }
          } catch {}
        }
        break;
        ...
        if (subId) {
          ...
          try {
            ...
            if (uid) {
              ...
              if (u?.email && (u.email_opt_in ?? 1)) await sendMail({ to: u.email, subject: "Sweat with Scott – Invoice paid", text: `Your latest invoice was paid successfully.` });
            }
          } catch {}
        }
        break;
      ...
      case "invoice.payment_failed": {
        ...
        if (subId) {
          ...
          try {
            ...
            if (uid) {
              const u = dbOperations.getUserById(uid) as any;
              if (u?.email && (u.email_opt_in ?? 1)) await sendMail({ to: u.email, subject: "Sweat with Scott – Payment failed", text: `Your recent payment failed. Please update your payment method in the billing portal.` });
            }
          } catch {}
        }
        break;
