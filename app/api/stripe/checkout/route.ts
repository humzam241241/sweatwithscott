...
const bodySchema = z.object({
  planCode: z.enum(["ADULT_UNLIMITED", "YOUTH_2X", "DROP_IN", "EIGHT_WEEK_RESET", "DAILY_DRILLS_MONTHLY"]),
...
export async function POST(req: Request) {
  try {
    ...
    const mode = planCode === "DROP_IN" || planCode === "EIGHT_WEEK_RESET" ? "payment" : "subscription";
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        ...(planCode === "DROP_IN"
          ? { class_instance_id: class_instance_id ? String(class_instance_id) : undefined }
          : {}),
      },
    });
